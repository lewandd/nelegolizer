...
class Test_from_mesh(unittest.TestCase):
    ...
    def test_returns_PolData(self):
        self.assertIsInstance(
            from_mesh(self.mesh,
                      voxel_mesh_shape=np.array([1, 1, 1])),
            pv.UnstructuredGrid)
...
class Test_from_grid(unittest.TestCase):
    ...
    def test_returns_ndarray_111(self):
        grid = np.array([[[True]]])
        self.assertIsInstance(
            from_grid(grid, voxel_mesh_shape=np.array([1, 1, 1])),
            pv.UnstructuredGrid)
    ...
    def test_correct_number_of_cells_1(self):
        grid = np.array([[[False, False], [False, False]],
                         [[True, False], [False, False]]])
        self.assertEqual(
            from_grid(grid, voxel_mesh_shape=np.array([1, 1, 1])).n_cells, 1)

    def test_correct_number_of_cells_3(self):
        grid = np.array([[[True, True], [False, False]],
                         [[True, False], [False, False]]])
        self.assertEqual(
            from_grid(grid, voxel_mesh_shape=np.array([1, 1, 1])).n_cells, 3)
