...
from nelegolizer.utils.mesh import get_resolution, translate_to_zero, get_position, scale_to
...
class Test_get_resolution(unittest.TestCase):
    ...
    def test_correct_values(self):
        ...
        self.assertTrue(np.allclose(np.array([xmax, ymax, zmax]), get_resolution(mesh)))

    def test_correct_values_with_translate_by_111(self):
        translate = np.array([1, 1, 1])
        translated_mesh = self.mesh.translate(translate) 
        self.assertTrue(np.allclose(get_resolution(translated_mesh), get_resolution(self.mesh)))
    ...
    def test_correct_scaled_values_by_111(self):
        ...
        self.assertTrue(np.allclose(get_resolution(scaled_mesh), get_resolution(self.mesh)))

    def test_correct_scaled_values_by_222(self):
        scale = np.array([2, 2, 2])
        scaled_mesh = self.mesh.scale(scale) 
        self.assertTrue(np.allclose(get_resolution(scaled_mesh), scale * get_resolution(self.mesh)))
    ...
    def test_correct_scaled_values_by_01_3_09_and_translation(self):
        ...
        self.assertTrue(np.allclose(get_resolution(res_mesh), scale * get_resolution(self.mesh)))

    def test_different_scaled_values_by_1_1_099(self):
        scale = np.array([1, 1, 0.99])
        scaled_mesh = self.mesh.scale(scale) 
        self.assertFalse(np.allclose(get_resolution(scaled_mesh), get_resolution(self.mesh)))
...
class Test_get_position(unittest.TestCase):
    ...
    def test_position_correct_after_translation_111(self):
        translate = np.array([1, 1, 1])
        translated_mesh = self.mesh.translate(translate) 
        self.assertTrue(np.allclose(get_position(translated_mesh), translate+get_position(self.mesh)))
    ...
    def test_position_incorrect_after_scaling(self):
        ...
        self.assertFalse(np.allclose(get_position(translated_mesh), get_position(self.mesh)))
...
class Test_translate_to_zero(unittest.TestCase):
    ...
    def test_correct_positions(self):
        self.assertTrue(np.allclose(get_position(translate_to_zero(self.mesh)), np.array([0, 0, 0])))
...
class Test_scale_to(unittest.TestCase):
    ...
    def test_returns_correct_resolution(self):
        ...
        self.assertTrue(np.allclose(get_resolution(scale_to(self.mesh, target_res)), target_res))
