...
from nelegolizer.utils.grid import find_best_rotation, rotate, get_subgrid, provide_divisibility, get_fill_ratio, from_pv_voxels, from_mesh, add_padding
...
class Test_rotate_grid(unittest.TestCase):
    ...
    def test_grid111_rotate_90(self):
            ...
            self.assertTrue(np.all(rotate(grid=grid111, degrees=90) == grid111_rot))

    def test_grid111_rotate_180(self):
        grid111 = np.array([[[False]]])
        grid111_rot = np.array([[[False]]])
        self.assertTrue(np.all(rotate(grid=grid111, degrees=180) == grid111_rot))

    def test_grid111_rotate_270(self):
        grid111 = np.array([[[True]]])
        grid111_rot = np.array([[[True]]])
        self.assertTrue(np.all(rotate(grid=grid111, degrees=270) == grid111_rot))
    ...
    def test_grid222_rotate_90(self):
        ...
        self.assertTrue(np.all(rotate(grid=grid222, degrees=90) == grid222_rot))
    ...
    def test_grid222_rotate_180(self):
        ...
        self.assertTrue(np.all(rotate(grid=grid222, degrees=180) == grid222_rot))
    ...
    def test_grid222_rotate_270(self):
        ...
        self.assertTrue(np.all(rotate(grid=grid222, degrees=270) == grid222_rot))
    ...
    def test_grid323_rotate_90(self):
        ...
        self.assertTrue(np.all(rotate(grid=grid323, degrees=90) == grid323_rot))
    ...
    def test_grid323_rotate_180(self):
        ...
        self.assertTrue(np.all(rotate(grid=grid323, degrees=180) == grid323_rot))
    ...
    def test_grid323_rotate_270(self):
        ...
        self.assertTrue(np.all(rotate(grid=grid323, degrees=270) == grid323_rot))
    ...
    def test_grid213_rotate_90(self):
        ...
        grid213_rot = np.array([[[True, False]],
                        [[False, True]],
                        [[False, False]]])
        self.assertTrue(np.all(rotate(grid=grid213, degrees=90) == grid213_rot))
    ...
    def test_grid213_rotate_180(self):
        ...
        grid213_rot = np.array([[[False, False, True]],
                        [[False, True, False]]])
        self.assertTrue(np.all(rotate(grid=grid213, degrees=180) == grid213_rot))
    ...
    def test_grid213_rotate_270(self):
        ...
        grid213_rot = np.array([[[False, False]],
                        [[True, False]],
                        [[False, True]]])
        self.assertTrue(np.all(rotate(grid=grid213, degrees=270) == grid213_rot))
...
class Test_extend(unittest.TestCase):
    ...
    def test_divisibility_222_by_342(self):
        ...
        expected342 = np.array([[[True, True], [True, True], [False, False], [False, False]],
...
class Test_get_fill_ratio(unittest.TestCase):
    ...
    def test_grid_222_fill_0(self):
        grid = np.array([[[False, False], [False, False]], [[False, False], [False, False]]])
        self.assertEqual(get_fill_ratio(grid), 0)

    def test_grid_222_fill_1(self):
        grid = np.array([[[True, True], [True, True]], [[True, True], [True, True]]])
        self.assertEqual(get_fill_ratio(grid), 1)

    def test_grid_222_fill_05(self):
        grid = np.array([[[True, False], [False, True]], [[False, False], [True, True]]])
        self.assertEqual(get_fill_ratio(grid), 0.5)

    def test_grid_252_fill_06(self):
        grid = np.array([[[True, False], [False, True], [False, True], [False, True], [False, True]], 
...
class Test_from_pv_voxels(unittest.TestCase):
    ...
    def test_result_is_ndarray1(self):
        self.assertTrue(isinstance(from_pv_voxels(self.pv_voxels1), np.ndarray))
                        
    def test_result_is_ndarray2(self):
        self.assertTrue(isinstance(from_pv_voxels(self.pv_voxels2), np.ndarray))
    ...
    def test_all_cells_are_same_shape1(self):
        first_cell_shape = umesh.get_resolution(self.pv_voxels1.extract_cells(0))
        for ind in range(self.pv_voxels1.n_cells):
            cell_shape = umesh.get_resolution(self.pv_voxels1.extract_cells(ind))
            self.assertTrue(np.allclose(cell_shape, first_cell_shape))

    def test_all_cells_are_same_shape2(self):
        first_cell_shape = umesh.get_resolution(self.pv_voxels2.extract_cells(0))
        for ind in range(self.pv_voxels2.n_cells):
            cell_shape = umesh.get_resolution(self.pv_voxels2.extract_cells(ind))
            self.assertTrue(np.allclose(cell_shape, first_cell_shape))
...
class Test_from_mesh(unittest.TestCase):
    ...
    def test_result_not_None(self):
        self.assertIsNotNone(from_mesh(self.mesh, voxel_mesh_shape=np.array([1,1,1])))

    def test_result_is_ndarray(self):
        self.assertIsInstance(from_mesh(self.mesh, voxel_mesh_shape=np.array([1,1,1])), np.ndarray)

class Test_add_padding(unittest.TestCase):
    def test_111_grid_padding_1_correct_values(self):
        grid = np.array([[[True]]])
        excepted = np.array([[[False, False, False], [False, False, False], [False, False, False]],
                              [[False, False, False], [False, True, False], [False, False, False]],
                              [[False, False, False], [False, False, False], [False, False, False]]])
        self.assertTrue(np.all(add_padding(grid, np.array([1, 1, 1])) == excepted))
    ...
    def test_112_grid_padding_1_correct_values(self):
        ...
        self.assertTrue(np.all(add_padding(grid, np.array([1, 1, 1])) == expected))
    ...
    def test_231_grid_padding_2_correct_values(self):
        ...
        for i in range(2):
            for j in range(3):
                for k in range(1):
                    self.assertEqual(grid[i, j, k], grid_with_padding[i+2, j+2, k+2])
