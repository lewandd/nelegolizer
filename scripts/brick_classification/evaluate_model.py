...
if __name__ == '__main__':
    argc = len(sys.argv) - 1
    if argc == 0:
        raise Exception("Usage: python3 create_model.py [model name] "
                        "[model2 name] ... or python3 create_model.py all")
    ...
    for arg in args:
        ...
        training_data = CustomVoxelsDataset(
            TRAIN_LABEL_FILE_PATH, TRAIN_DATA_DIR)
        test_data = CustomVoxelsDataset(
            TEST_LABEL_FILE_PATH, TEST_DATA_DIR)
